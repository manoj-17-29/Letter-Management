const firebaseConfig = {
  ...
  function createRow(dataKey, dataValue) {
    ...
    newRow.innerHTML = "<td>" + dataValue.odname + "</td><td>" + dataValue.odrollno + "</td><td>" + dataValue.odyear + "</td><td>" + dataValue.odsec + "</td><td>" + dataValue.odleavedays + "</td><td>" + dataValue.odstartingday + "</td><td>" + dataValue.odendingday + "</td><td>" + dataValue.odreason + "</td><td>    <button class='btn btn-primary btn-sm fa-solid fa-images' onclick='viewimage(\"" + dataKey + "\", \"" + dataValue.imageUrl + "\")'></button></td><td>  <button class='btn btn-danger btn-sm fa-solid fa-trash' onclick='deleteRow(\"" + dataKey + "\")'></button></td>";
  ...
  function deleteRow(dataKey) {
    database.ref("od_data/" + dataKey).remove();
  }

  database.ref("od_data").on("child_added", function (snapshot) {
    var dataKey = snapshot.key;
    var dataValue = snapshot.val();
    createRow(dataKey, dataValue);
  });

  database.ref("od_data").on("child_removed", function (snapshot) {
    var dataKey = snapshot.key;
